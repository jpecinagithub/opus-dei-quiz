import { Question } from './types';

export const GUADALUPE_QUESTIONS_POOL: Question[] = [
  {
    id: 1001,
    text: '¿En qué ciudad nació Guadalupe Ortiz de Landázuri en 1916?',
    options: ['Madrid', 'Barcelona', 'Sevilla', 'Bilbao'],
    correctAnswer: 0
  },
  {
    id: 1002,
    text: '¿En qué carrera universitaria se licenció Guadalupe en la Universidad Central de Madrid?',
    options: ['Física', 'Matemáticas', 'Química', 'Biología'],
    correctAnswer: 2
  },
  {
    id: 1003,
    text: '¿En qué año conoció Guadalupe a San Josemaría Escrivá?',
    options: ['1936', '1944', '1950', '1928'],
    correctAnswer: 1
  },
  {
    id: 1004,
    text: '¿Cuál era el mensaje clave que Guadalupe difundió tras unirse al Opus Dei?',
    options: [
      'La santidad solo es para religiosos',
      'La santidad se vive en lo cotidiano y el trabajo profesional',
      'La ciencia y la fe son incompatibles',
      'El retiro del mundo es necesario para ser santo'
    ],
    correctAnswer: 1
  },
  {
    id: 1005,
    text: '¿En qué país vivió Guadalupe entre 1950 y 1956 para impulsar el Opus Dei?',
    options: ['Argentina', 'México', 'Colombia', 'Perú'],
    correctAnswer: 1
  },
  {
    id: 1006,
    text: '¿Qué tipo de iniciativas impulsó Guadalupe especialmente en México?',
    options: [
      'Solo centros de investigación pura',
      'Iniciativas educativas y sociales para mujeres',
      'Fábricas de productos químicos industriales',
      'Hospitales de alta complejidad'
    ],
    correctAnswer: 1
  },
  {
    id: 1007,
    text: '¿Cuál de estos rasgos de personalidad destacaban quienes conocieron a Guadalupe?',
    options: ['Seriedad extrema', 'Alegría constante y naturalidad', 'Timidez paralizante', 'Actitud distante'],
    correctAnswer: 1
  },
  {
    id: 1008,
    text: '¿Qué problema de salud sufrió Guadalupe de forma importante en sus últimos años?',
    options: ['Problemas de visión', 'Problemas cardíacos', 'Problemas de movilidad', 'Problemas respiratorios'],
    correctAnswer: 1
  },
  {
    id: 1009,
    text: '¿En qué año fue beatificada Guadalupe Ortiz de Landázuri?',
    options: ['2000', '2010', '2019', '2022'],
    correctAnswer: 2
  },
  {
    id: 1010,
    text: '¿Qué hito marcó la beatificación de Guadalupe en el Opus Dei?',
    options: [
      'Fue la primera mujer laica en ser beatificada',
      'Fue la primera científica en ser beatificada',
      'Fue la primera persona de Madrid en ser beatificada',
      'Fue la primera profesora en ser beatificada'
    ],
    correctAnswer: 0
  },
  {
    id: 1011,
    text: '¿En qué ciudad falleció Guadalupe en 1975?',
    options: ['Madrid', 'México D.F.', 'Pamplona', 'Roma'],
    correctAnswer: 2
  },
  {
    id: 1012,
    text: '¿Qué defendía Guadalupe sobre el trabajo profesional?',
    options: [
      'Que es un castigo necesario',
      'Que el trabajo bien hecho aporta valor a la sociedad',
      'Que es secundario frente a la vida espiritual',
      'Que solo importa el éxito económico'
    ],
    correctAnswer: 1
  },
  {
    id: 1013,
    text: '¿Cuál fue uno de los objetivos de su trabajo social en México?',
    options: [
      'Promover la dignidad y autonomía femenina',
      'Construir iglesias monumentales',
      'Exportar productos españoles',
      'Organizar eventos políticos'
    ],
    correctAnswer: 0
  },
  {
    id: 1014,
    text: '¿Cómo se describe el liderazgo de Guadalupe según su legado?',
    options: ['Liderazgo autoritario', 'Liderazgo con impacto social', 'Liderazgo pasivo', 'Liderazgo puramente académico'],
    correctAnswer: 1
  },
  {
    id: 1015,
    text: '¿Qué representa Guadalupe para las mujeres en la ciencia?',
    options: [
      'Que la ciencia no es para mujeres',
      'Un referente femenino en ciencia en el siglo XX',
      'Que hay que elegir entre ciencia y familia',
      'Que la ciencia es solo para hombres'
    ],
    correctAnswer: 1
  },
  {
    id: 1016,
    text: '¿En qué universidad ejerció como profesora de química?',
    options: ['Universidad de Oxford', 'Universidad Central de Madrid', 'Universidad de Harvard', 'Universidad de Salamanca'],
    correctAnswer: 1
  },
  {
    id: 1017,
    text: '¿Cómo era el estilo de vida de Guadalupe a pesar de sus cargos?',
    options: ['Lujoso y ostentoso', 'Sencillo, práctico y resolutivo', 'Complicado y burocrático', 'Aislado del mundo'],
    correctAnswer: 1
  },
  {
    id: 1019,
    text: '¿Qué buscaba Guadalupe con sus iniciativas para personas con menos recursos?',
    options: ['Crear oportunidades reales', 'Mantener el status quo', 'Fomentar la dependencia', 'Ganar votos'],
    correctAnswer: 0
  },
  {
    id: 1020,
    text: '¿Qué cualidad intelectual destacaba en Guadalupe durante sus estudios?',
    options: ['Inteligencia y rigor', 'Memoria fotográfica', 'Improvisación constante', 'Desinterés por el detalle'],
    correctAnswer: 0
  },
  {
    id: 1021,
    text: '¿En qué año nació Guadalupe?',
    options: ['1910', '1916', '1920', '1925'],
    correctAnswer: 1
  },
  {
    id: 1022,
    text: '¿Cuál era la profesión de Guadalupe antes de dedicarse plenamente al Opus Dei?',
    options: ['Médico', 'Química y profesora', 'Arquitecta', 'Abogada'],
    correctAnswer: 1
  },
  {
    id: 1023,
    text: '¿Qué impulsó Guadalupe en la zona rural de México (Morelos)?',
    options: ['Una mina de plata', 'Un centro de formación para campesinas (Montefalco)', 'Un hotel de lujo', 'Una pista de aterrizaje'],
    correctAnswer: 1
  },
  {
    id: 1024,
    text: '¿Cómo afrontó Guadalupe su enfermedad cardíaca?',
    options: ['Con amargura', 'Con optimismo y entrega a los demás', 'Dejando de trabajar por completo', 'Quejándose constantemente'],
    correctAnswer: 1
  },
  {
    id: 1025,
    text: '¿Qué significa que Guadalupe fuera una de las "primeras mujeres" del Opus Dei?',
    options: [
      'Que fue la fundadora',
      'Que se incorporó en los inicios de la sección femenina',
      'Que fue la única mujer durante años',
      'Que no tuvo contacto con otras mujeres'
    ],
    correctAnswer: 1
  },
  {
    id: 1026,
    text: '¿En qué rama de la química trabajó Guadalupe en su etapa investigadora?',
    options: ['Química aplicada', 'Química nuclear', 'Química cuántica', 'Petroquímica'],
    correctAnswer: 0
  },
  {
    id: 1028,
    text: '¿Dónde se celebró la ceremonia de beatificación de Guadalupe?',
    options: ['Roma', 'México', 'Madrid', 'Pamplona'],
    correctAnswer: 2
  },
  {
    id: 1029,
    text: '¿Qué tipo de formación promovía para las mujeres en México?',
    options: ['Integral: profesional, humana y espiritual', 'Solo técnica', 'Solo doméstica', 'Solo deportiva'],
    correctAnswer: 0
  },
  {
    id: 1030,
    text: '¿Cómo era la relación de Guadalupe con San Josemaría?',
    options: ['De desconfianza', 'De profunda sintonía espiritual y filial', 'Inexistente', 'Puramente administrativa'],
    correctAnswer: 1
  },
  {
    id: 1033,
    text: '¿En qué década comenzó Guadalupe su labor en México?',
    options: ['Años 30', 'Años 40', 'Años 50', 'Años 60'],
    correctAnswer: 2
  },
  {
    id: 1034,
    text: '¿Qué legado dejó Guadalupe en el ámbito científico?',
    options: [
      'Un ejemplo de integración entre vida profesional y valores',
      'Varios premios Nobel',
      'El descubrimiento de un nuevo elemento',
      'La invención de la penicilina'
    ],
    correctAnswer: 0
  },
  {
    id: 1035,
    text: '¿Cómo se llamaba el centro de formación que impulsó en México?',
    options: ['Montefalco', 'Torreciudad', 'La Moncloa', 'Altoclaro'],
    correctAnswer: 0
  },
  {
    id: 1037,
    text: '¿Qué importancia tenía para ella la formación profesional?',
    options: ['Mucha: era clave para la autonomía de las personas', 'Poca: solo importaba rezar', 'Ninguna', 'Solo para hombres'],
    correctAnswer: 0
  },
  {
    id: 1038,
    text: '¿En qué año terminó su etapa en México?',
    options: ['1950', '1956', '1960', '1975'],
    correctAnswer: 1
  },
  {
    id: 1041,
    text: '¿Cuál era el apellido completo de Guadalupe?',
    options: ['Escrivá', 'Ortiz de Landázuri', 'Fernández', 'García'],
    correctAnswer: 1
  },
  {
    id: 1042,
    text: '¿Qué tipo de trabajo desarrolló en el ámbito de la química?',
    options: ['Investigación y docencia', 'Solo ventas', 'Solo limpieza de laboratorios', 'Ninguno'],
    correctAnswer: 0
  },
  {
    id: 1043,
    text: '¿Cómo influyó su fe en su carrera científica?',
    options: [
      'La hizo abandonar la ciencia',
      'Le dio un propósito de servicio y excelencia',
      'No tuvo ninguna influencia',
      'La hizo ser menos rigurosa'
    ],
    correctAnswer: 1
  },
  {
    id: 1044,
    text: '¿Qué hito social logró en México para las mujeres rurales?',
    options: [
      'Acceso a formación técnica y humana',
      'Derecho al voto',
      'Propiedad de tierras',
      'Servicio militar obligatorio'
    ],
    correctAnswer: 0
  },
  {
    id: 1046,
    text: '¿Qué importancia tiene su beatificación para los laicos?',
    options: [
      'Muestra que un laico puede llegar a los altares por su vida ordinaria',
      'Ninguna',
      'Que hay que ser cura para ser santo',
      'Que solo los científicos son santos'
    ],
    correctAnswer: 0
  },
  {
    id: 1047,
    text: '¿En qué siglo desarrolló Guadalupe la mayor parte de su actividad?',
    options: ['Siglo XIX', 'Siglo XX', 'Siglo XXI', 'Siglo XVIII'],
    correctAnswer: 1
  },
  {
    id: 1048,
    text: '¿Qué actitud tenía hacia el futuro de las mujeres?',
    options: ['Pesimista', 'Promotora de su dignidad y capacitación', 'Indiferente', 'Restrictiva'],
    correctAnswer: 1
  },
  {
    id: 1049,
    text: '¿Cuál es la "idea clave" de su vida?',
    options: [
      'Carrera exigente sin perder propósito ni valores',
      'Éxito a cualquier precio',
      'Abandono de la vida profesional por la fe',
      'Vivir sin trabajar'
    ],
    correctAnswer: 0
  },
  {
    id: 1050,
    text: '¿Qué sentimiento predominaba en Guadalupe incluso ante la muerte?',
    options: ['Miedo', 'Paz y alegría', 'Rencor', 'Desesperación'],
    correctAnswer: 1
  },
  {
    id: 1051,
    text: '¿En qué fecha nació Guadalupe Ortiz de Landázuri?',
    options: ['12 de diciembre de 1916', '12 de diciembre de 1914', '18 de mayo de 1916', '16 de julio de 1916'],
    correctAnswer: 0
  },
  {
    id: 1052,
    text: '¿En qué fecha falleció Guadalupe Ortiz de Landázuri?',
    options: ['16 de julio de 1975', '26 de junio de 1975', '18 de mayo de 2019', '12 de diciembre de 1975'],
    correctAnswer: 0
  },
  {
    id: 1053,
    text: '¿En qué año se trasladó Guadalupe a México para impulsar la labor del Opus Dei con mujeres?',
    options: ['1944', '1950', '1956', '1960'],
    correctAnswer: 1
  },
  {
    id: 1054,
    text: '¿En qué año dejó México para ayudar en Roma?',
    options: ['1954', '1956', '1958', '1960'],
    correctAnswer: 1
  },
  {
    id: 1055,
    text: '¿En qué fecha fue beatificada Guadalupe Ortiz de Landázuri?',
    options: ['18 de mayo de 2019', '18 de noviembre de 2001', '12 de diciembre de 2019', '25 de mayo de 2004'],
    correctAnswer: 0
  },
  {
    id: 1056,
    text: '¿En qué instituto terminó el bachillerato Guadalupe?',
    options: ['Instituto Miguel de Cervantes', 'Instituto San Isidro', 'Instituto Cardenal Cisneros', 'Instituto Ortega y Gasset'],
    correctAnswer: 0
  },
  {
    id: 1057,
    text: '¿En qué año comenzó sus estudios en la Facultad de Ciencias Químicas de la Universidad Central?',
    options: ['1931', '1933', '1935', '1937'],
    correctAnswer: 1
  },
  {
    id: 1058,
    text: '¿Cómo se llamaba el padre de Guadalupe?',
    options: ['Manuel Ortiz de Landázuri', 'José Ortiz de Landázuri', 'Antonio Fernández', 'Eduardo Ortiz'],
    correctAnswer: 0
  }
];
